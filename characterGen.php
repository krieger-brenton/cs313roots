  ...
  <body>
    ...
    <div class="master"><?php require $_SERVER['DOCUMENT_ROOT'] . '/files_php/characterGen.php';?>
      ...
      <div class="characterCombatInfo">
        <div class="characterAC"><span><?php print "Armor Class: " . $_SESSION['ac']; ?></span></div>
        <div class="characterShield"><span><?php print "Shield Equiped: "; if ($_SESSION['shield']) print "Yes"; else print "No"; ?></span></div>
        <div class="characterHitDice"><span><?php print "Hit Dice: " . $_SESSION['hit_die']; ?></span></div>
        <div class="characterProffBonus"><span><?php print "Profficiency Bonus: " . $_SESSION['prof']; ?></span></div>
        <div class="weapon">
          <div class="weaponInfo">
            <?php print "Weapon Name: " . ucwords($_SESSION['weapon_one_name']); ?>
            <?php print "Damage: " . ucwords($_SESSION['weapon_one_damage']); ?>
            <?php print "Damage Type: " . ucwords($_SESSION['weapon_one_damage_type']); ?>
          </div>
          <button class="weaponAttack">not ready yet</button><br><br><br>
          <div class="weaponContent">
             
            <?php foreach($props_one as $prop){ print $prop . '<BR />';} ?>
          </div>
        </div>
        <div class="weapon">
          <div class="weaponInfo">
            <?php print "Weapon Name: " . ucwords($_SESSION['weapon_two_name']); ?>
            <?php print "Damage: " . ucwords($_SESSION['weapon_two_damage']); ?>
            <?php print "Damage Type: " . ucwords($_SESSION['weapon_two_damage_type']); ?>
          </div>
          <button class="weaponAttack">not ready yet</button><br><br><br>
          <div class="weaponContent">
             
            <?php foreach($props_two as $prop){ print $prop . '<BR />';} ?>
          </div>
        </div>
      </div>
      <div onload="checkCharacterMod()" class="characterStats">
        <div class="characterStat">
          <label>Strength <span class="characterMod"><?php if ($_SESSION['str'] > 0) print "Modifier: +" . $_SESSION['str']; ?></span>
            <input type="button" value="not ready"><span><?php if (!empty($_GET['str'])) print $_GET['str']; ?></span>
          </label>
        </div>
        <div class="characterStat">
          <label>Dexterity <span class="characterMod"><?php if ($_SESSION['dex'] > 0) print "Modifier: +" . $_SESSION['dex']; ?></span>
            <input type="button" value="not ready"><span><?php if (!empty($_GET['dex'])) print $_GET['dex']; ?></span>
          </label>
        </div>
        <div class="characterStat">
          <label>Constitution<span class="characterMod"><?php if ($_SESSION['con'] > 0) print "Modifier: +" . $_SESSION['con']; ?></span>
            <input type="button" value="not ready"><span><?php if (!empty($_GET['con'])) print $_GET['con']; ?></span>
          </label>
        </div>
        <div class="characterStat">
          <label>Intelligence<span class="characterMod"><?php if ($_SESSION['int'] > 0) print "Modifier: +" . $_SESSION['int']; ?></span>
            <input type="button" value="not ready"><span><?php if (!empty($_GET['int'])) print $_GET['int']; ?></span>
          </label>
        </div>
        <div class="characterStat">
          <label>Wisdom<span class="characterMod"><?php if ($_SESSION['wis'] > 0) print "Modifier: +" . $_SESSION['wis']; ?></span>
            <input type="button" value="not ready"><span><?php if (!empty($_GET['wis'])) print $_GET['wis']; ?></span>
          </label>
        </div>
        <div class="characterStat">
          <label>Charisma <span class="characterMod"><?php if ($_SESSION['cha'] > 0) print "Modifier: +" . $_SESSION['cha']; ?></span>
